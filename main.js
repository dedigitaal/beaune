...
function initMobileMenu() {
  ...
  const closeActiveSub = (instant = false) => {
    ...
    if (sub) {
      ...
      else gsap.to(sub, { height: 0, duration: CLOSE_D, ease: EASE });
    }
    activeSub = null;
  };
  ...
  const openMenu = () => {
    ...
    gsap.to(menuIcon, { rotate: 90, duration: 0.3, ease: EASE });
  ...
  const closeMenu = (instant = false) => {
    ...
    if (instant) {
      ...
      gsap.to(menuIcon, { rotate: 0, duration: 0.3, ease: EASE });
      gsap.to(wrapper, { height: 0, duration: CLOSE_D, ease: EASE });
    }
  };
  ...
  KEYS.forEach(k => {
    ...
    const onClick = (e) => {
      e.preventDefault();
      if (activeSub === k) {
        gsap.to(sub, { height: 0, duration: CLOSE_D, ease: EASE });
        activeSub = null;
        return;
      }
      if (activeSub) {
        const prev = nextPage.querySelector(`.submenu-mobiel.${activeSub}`);
        if (prev) gsap.to(prev, { height: 0, duration: CLOSE_D, ease: EASE });
      }
      gsap.to(sub, { height: 'auto', duration: OPEN_D, ease: EASE });
      activeSub = k;
    };
